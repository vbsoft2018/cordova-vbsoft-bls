...
public class VBSoftBls extends CordovaPlugin {
    ...
    public static final int MESSAGE_DEVICE_INFO = 7;
    public static final int MESSAGE_RE_LISTEN = 8;
             ...
             switch (msg.what) {
                 ...
                 case MESSAGE_READ_RAW:
                    if (rawDataAvailableCallback != null) {
                        ...
                        if(D) Log.i(TAG, "MESSAGE_READ_RAW: " + bytes);
                ...
                case MESSAGE_DEVICE_NAME:
                    ...
                    break;
                case MESSAGE_RE_LISTEN:
                    if(listenCallback!=null){
                        Log.i(TAG, "listenCallback::Send");
                        PluginResult result = new PluginResult(PluginResult.Status.OK,"relisten");
                        result.setKeepCallback(true);
                        listenCallback.sendPluginResult(result);
                    }
                    break;
                case MESSAGE_DEVICE_INFO:
                    if(listenCallback!=null){
                        Log.i(TAG, "listenCallback::Send");
                        PluginResult result = new PluginResult(PluginResult.Status.OK, msg.getData().getString(DEVICE_INFO));
                        result.setKeepCallback(true);
                        listenCallback.sendPluginResult(result);
                    }
                    Log.i(TAG, msg.getData().getString(DEVICE_INFO));
                    break;
                case MESSAGE_TOAST:
